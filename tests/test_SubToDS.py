...
def testCreateDS():
    ...
    ds_subscriber_account3 = accounts[5] #will subscribe to the "Tetris" dataset with a 1day sub time
    ds_subscriber_account4 = accounts[6] #will also subscribe to the "Tetris" dataset with a 30 days sub time

    #Fund accounts
    DHN.transfer(ds_creator_account1, 30, {"from": dohrnii_account}) #fund the creator
    DHN.transfer(ds_creator_account2, 30, {"from": dohrnii_account}) #fund the creator
    ...
    DHN.transfer(ds_subscriber_account3, 30, {"from": dohrnii_account}) #fund sub3
    DHN.transfer(ds_subscriber_account4, 30, {"from": dohrnii_account}) #fund sub4    

    #Create a DS and instantiate it
    deployer.createDS(DHN, DSF, ds_creator_account1,"Tetris", "https://ipfs.io/ipfs/Qme7ss3ARVgxv6rXqVPiikMJ8u2NLgmgszg13pYrDKEoiu",
                 "Games","Tetris statistics and data", 10, 3600, 2)

    DS_instance1 = deployer.getDSbyName(DSF, ds_subscriber_account1, "Tetris")
    
    #Create a DS and instantiate it
    deployer.createDS(DHN, DSF, ds_creator_account2,"Desserts", "https://ipfs.io/ipfs/Qme7ss3ARVgxv6rXqVPiikMJ8u2NLgmgszg13pYrDKEoiu",
                 "Food","Some dessert recipes", 5, 3600, 2)
          
    DS_instance2 = deployer.getDSbyName(DSF, ds_subscriber_account2, "Desserts")

    #Sub1
    deployer.subToDS(DHN, DSF, ds_subscriber_account1, "Tetris", 0)    
    #Sub2
    deployer.subToDS(DHN, DSF, ds_subscriber_account2, "Tetris", 0)
    #Sub3
    deployer.subToDS(DHN, DSF, ds_subscriber_account3, "Desserts", 1)    
    #Sub4
    deployer.subToDS(DHN, DSF, ds_subscriber_account4, "Desserts", 2)

#Assertion: DS creation alterations
    
    assert (20+10*2, 2) == (DHN.balanceOf(DS_instance1),#contract balance changes because of 2 subs
                            DS_instance1.subCount())#subcount increases by 2
    
    assert (20+2*5,2) == (DHN.balanceOf(DS_instance2),#contract balance changes because of 2 subs
...
#Assertion: Sub info
    ...
    assert (10, 1, True) == (info1[0], info1[1], info1[3])

    #Subscribed to "Tetris"
    info2 = DS_instance1.addressToSub(ds_subscriber_account2)
    #Price paid, subscription time, Is this person currently subbed?
    assert (10, 1, True) == (info2[0], info2[1], info2[3])
    ...
    #Price paid, subscription time, Is this person currently subbed?
    assert (5, 24*3600, True) == (info4[0], info4[1], info4[3])

    #Subscribed to "Desserts"
    info5 = DS_instance2.addressToSub(ds_subscriber_account4)
    #Price paid, subscription time, Is this person currently subbed?
    assert (5, 30*24*3600, True) == (info5[0], info5[1], info5[3])
